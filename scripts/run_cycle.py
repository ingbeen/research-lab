...
#
# [실측 2026-09-23] 모델을 바꾸자(`agent/invoke.py` 의 MODEL) 토큰당 가격이 두 배가 되었다.
# 두 회차의 최대는 반증 $3.0299 였지만, 위에서 $2.4330 에 잘린 반증은 필요량이 그 이상이었으므로
# 새 가격으로는 **$4.866 이상**이다. 알려진 가장 무거운 단계에 같은 1.6배 규칙을 걸어 $8 이다.
# **모델을 바꾸면 이 값도 다시 잰다.**
#
...
#
# [실측 2026-09-15] 근거 문서 한 장이 5시간 창의 약 23% 다. [실측 2026-09-23] 모델을 바꾼 뒤에는
# 25.6% · 27.5% 로, 네 장이면 한 창의 102 ~ 110% — 한 창을 채우고 조금 넘친다.
