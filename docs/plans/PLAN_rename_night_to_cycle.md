...
- [ ] 목표 1: 실행 단위를 가리키는 「밤」을 **「회차」**로 바꾼다 — 저장소가 낮에도 돌아가므로 시간을 함축하는 이름이 사실과 맞지 않는다
- [ ] 목표 2: 영문 식별자 `night` 계열을 **`cycle`** 로 바꾼다 — 파일·심볼·테스트 함수명 포함
- [ ] 목표 3: 리서치 스킬 폴더 `night-research` 를 **`dossier-research`** 로 바꾸고 그 참조를 전부 따라 고친다
- [ ] 목표 4: 「밤」이 안 들어가지만 같은 시간 가정을 깐 말(「아침에 보면」·「매일」·「어제」)을 시간 중립 표현으로 바꾼다
...
- [ ] 기능 요구사항 충족 — 아래 넷을 `grep` 으로 실측해 적는다
      ① `src/` · `scripts/` · `tests/` · `docs/`(plans 제외) · 설정 파일에 「밤」 0곳
      ② 같은 범위에 `night` 0곳
      ③ `runs/` 와 `docs/plans/` 의 곳 수는 **착수 전과 동일**
      ④ 「야간」 8곳은 **그대로**
- [ ] 회귀/신규 테스트 추가 — 새 테스트는 만들지 않는다. 기존 테스트의 심볼·파일명·문자열 assert 를 따라 고친다
