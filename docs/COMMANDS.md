...
매일 01:00 에 회차를 띄우는 예약을 끄고 켭니다. **회차를 직접 돌리지는 않습니다** —
지금 한 장을 뽑고 싶으면 위 「회차 실행」을 씁니다.

```bash
# 지금 도는지 · 재부팅 후에도 도는지 본다
./scripts/trigger.sh status

# 예약을 끈다 (재부팅해도 꺼진 채로 유지됩니다)
./scripts/trigger.sh off

# 예약을 켠다
./scripts/trigger.sh on
```

### 🔴 `launchctl bootout` 만으로는 «안 꺼집니다»

끄는 데 두 동작이 필요하고, **하나만 하면 절반만 꺼지는데 에러가 나지 않습니다.**
...
`./scripts/trigger.sh off` 가 둘을 같이 걸고 **결과를 다시 읽어 검증**합니다.
되살릴 때는 `enable` 이 `bootstrap` 보다 **먼저**여야 합니다(순서가 반대면
`Bootstrap failed: 5`). `on` 이 그 순서를 지킵니다.

> 🔴 **`launchctl list` 로 켜졌는지 판정하지 마세요.** `disabled` 인 서비스도 그대로
> 보여줍니다. 실제로 그 출력만 보고 「내려갔다」고 판정했다가 사흘간 회차가 도는 것을
> 놓쳤습니다 — `status` 가 「지금」과 「재부팅 후」를 갈라 보여주는 이유입니다.
>
> **절전 해제 예약(`pmset`)은 건드리지 않고 보여주기만 합니다.** `sudo` 가 필요해
> 사람 몫이며, 근거는 [OPERATIONS.md](OPERATIONS.md) 3.3 에 있습니다.
>
> 🔴 **도는 중인 회차가 있으면 `off` 가 경고합니다.** 예약을 내리면 그 회차가 끊길 수 있고,
> 끊긴 회차는 `runs/cycles.jsonl` 에 종료 줄을 남기지 못해 **나중에 보면 전원 차단과
> 구별되지 않습니다.** 경고만 하고 막지는 않습니다.
