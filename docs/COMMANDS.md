...
# 끊긴 밤을 이어받는다 (실행 폴더를 지정하면 그 자리에서 이어진다)
...
```bash
poetry run python scripts/run_night.py --budget-usd 0.5
```
...
| --- | --- | --- |
| `0` | 완주 | 없음 |
| `1` | 미완성 (그 외 실패) | 없음. **다음 밤이 이어받습니다** |
| `1` + stderr 에 `[막힘]` | 같은 단계가 **3밤 연속** 막혀 그 실행 폴더를 접었습니다 | 🔴 **원인을 고쳐야 합니다.** 후보가 원장에 `- [!]` 로 남으므로, 고친 뒤 `- [ ]` 로 바꾸고 사유 줄을 지우면 다시 팝니다. 안 고치면 또 같은 자리에서 막힙니다 |
