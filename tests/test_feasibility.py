...
[중요] **카탈로그가 없어도 그 회차는 돈다.** 카탈로그는 검사기가 아니라 «입력»이고,
입력이 없다고 회차를 멈추면 검사기가 파이프라인을 세우는 구조가 된다 — 계층 계약 §3 의
...
def test_prompt_demands_self_standing_prose() -> None:
    ...
    이 저장소의 1순위 제약이다 — dossier 는 저장소 밖으로 나가므로 카탈로그를 가리키는 말로
    적히면 그 순간 **판단이 불가능한 종이**가 된다. 자립성 게이트는 조사가 붙은 꼴(「카탈로그에 …」
    류)만 잡고 다른 말로 가리키면 못 본다. 그래서 그 요구가 사라지지 않게 박는다.

    [주의] 요구를 «금지 표현을 인용해» 적지 않는다. 지시문이 금지 표현을 쓰면 그것은
    지시가 아니라 예시가 된다 — `test_prompt_selfcontained.py` 가 그 계약을 고정한다.

    [중요] 없는 것을 적을 때 쓸 말도 «자립 문장»으로 준다. 에이전트는 받은 말을 되돌려 쓰므로,
    그 말이 어디를 가리키는 꼴이면 어떤 이름이든 그대로 새어 나간다.

    Given: 지시문
    When: 만든다
    Then: 가리키지 말고 내용을 풀어 적으라는 요구와, 없는 것을 적을 자립 문장이 들어 있다
    """
    prompt = feasibility.build_prompt(CLAIM, CATALOG)

    assert "답이 아래를 가리키게 쓰지 마세요" in prompt
    assert "그 내용을 답 안에 풀어 적습니다" in prompt
    assert "이 파이프라인이 실측한 기록 없음" in prompt


def test_missing_catalog_does_not_stop_the_cycle(tmp_path: Path) -> None:
    """
    목적: [중요] 카탈로그를 못 읽어도 그 회차가 «멈추지 않는» 계약을 고정한다.

    카탈로그는 검사기가 아니라 입력이다. 없다고 회차를 세우면 **문서 한 장이 파이프라인을
...
def test_run_carries_the_catalog_into_the_prompt(tmp_path: Path) -> None:
    ...
    회차에는 카탈로그가 빠진다.** 그 구멍은 실측 전까지 드러나지 않는다.
...
def test_without_a_pinned_candidate_it_is_an_invariant_violation(tmp_path: Path) -> None:
    ...
    후보 없이 돌아 상한까지 헛돈 뒤 「다음 회차가 이어받습니다」로 보고된다.
...
def test_cost_and_measurements_survive_a_block(tmp_path: Path) -> None:
    ...
    막힌 회차의 비용이 안 남으면 회차 예산을 정할 때 그만큼이 통째로 빠진 값으로 계산된다.
...
def test_unknown_catalog_hits_are_counted_separately(tmp_path: Path) -> None:
    ...
    계측의 진실성이고, 게이트를 세우면 항목명을 못 옮겨 적은 회차가 통째로 죽는다.
