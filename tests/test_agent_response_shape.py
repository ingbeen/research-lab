...
불러 받은 실제 응답이다. [미검증]으로 남아 있던 두 가지를 이 응답 하나가 닫았다.
...
def test_usage_components_survive_the_parse() -> None:
    ...
    [중요] 바로 위 테스트가 고정한 「캐시 읽기를 빼고 센다」는 합계일 뿐이다. 합계만 남기면
    가중치가 바뀌는 날 **다시 계산할 수가 없다.** 그래서 이 계층은 **판정하지 않고
...
def test_measured_session_limit_classifies_as_limit() -> None:
    ...
    [실측 2026-09-12] 「일부러 만들 수 없다」고 [미검증] 으로 남아 있었는데
