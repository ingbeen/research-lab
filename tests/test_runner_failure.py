...
- **한도 소진** — 재시도해봐야 또 막힌다. 깨끗이 끝내고 다음 회차가 이어받는다
...
def test_classifier_never_raises_on_odd_input() -> None:
    ...
    분류기가 예외를 올리면 그 회차가 통째로 끝난다 — 고칠 수 있었던 실패까지 함께.
...
def test_retry_limit_exists_and_is_finite() -> None:
    ...
    상한이 없으면 한 회차 내내 같은 실패를 반복하며 토큰을 태운다. 나중에 보면
...
def test_budget_is_not_confused_with_the_subscription_limit() -> None:
    ...
    구독 한도는 기다리면 풀리고 그 회차는 정상 종료다. 예산 상한은 **사람이 값을 올리기
    전에는 안 풀린다** — 둘을 같은 갈래로 두면 「정상입니다」로 보고되면서 회차마다 멈춘다.
