...
def test_first_cycle_starts_at_the_first_step() -> None:
    """
...
def test_returns_none_when_every_step_is_done() -> None:
    """
    목적: 다 끝난 회차를 「할 일 없음」으로 다루는 계약을 고정한다.
...
def test_step_names_are_stable_identifiers() -> None:
    ...
    이 이름은 파일에 기록돼 다음 회차가 읽는다. 표시용 문구가 아니므로 영문 식별자로 두고,
