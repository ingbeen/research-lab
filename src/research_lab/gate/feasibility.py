...
이 칸들을 자유 서술로 두면 **찾은 것만 쓰고 안 찾은 것은 언급조차 안 한다.** 「1월 효과」를
채워 본 설계 예시에서 실제로 문제를 잡아낸 칸이 5번(「신호가 연 1회」)이었고, 그것은
**칸이 정해져 있어서 안 물을 수가 없었기 때문에** 나왔다. 그래서 4·5번 칸이 묻는 것을
하나씩 자리로 요구한다.
...
사전이 실측으로 뒤집은 함정에 다시 선다 — 그 사전이 기각 목록이었다면 진짜 원장의 후보
13개 중 12개가 죽었다. 기각의 대가가
오탐 중 가장 크고, 이 혼입은 「없는 출처」와 달리 **읽으면 사람이 바로 본다.**
판정 단계가 이 칸을 읽으므로 섞인 값은 결론에 닿을 수 있다 — 그 정도를 가늠할 재료가 이 셈이다.
"""
...
#
# [중요] 자리마다 안 물으면 생기는 일이 있다 — 미래 참조를
...
# **다른 물음이다.** 미국장이라 한국시간 새벽에만 집행되는 것과,
...
# 기준선 항목이다. 둘을 섞으면 5번 칸이 금지된 값을 들고 결론을 만들게 된다
COST_TERMS: Final = (
    "수수료",
    "세금",
    "세율",
...
def shortfall_reason(payload: Mapping[str, Any]) -> str | None:
    ...
    if not is_filled(payload.get(KEY_MARKET)):
        ...
        # 전에는 기각이 아니다. 시장 없이 적힌 판정은 나중에 되짚을 수도 없다
...
def cost_terms_in(section: Any) -> tuple[str, ...]:
    ...
    [중요] 이 값으로 막지 않는다. 5번 칸에서 비용을 묻지 않는 것은
