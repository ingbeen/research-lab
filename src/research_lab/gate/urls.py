...
밤이 매번 죽는다** — 판정을 «못 하는 것»과 «실패로 판정하는 것»은 다르다.

[주의] 그래서 남는 구멍이 하나 있다 — **도메인 자체가 가짜인 URL 은 DNS 실패로 떨어져
통과한다.** 실제 위조는 대개 「진짜 도메인 + 가짜 경로」 모양이라 404 로 잡히지만,
DNS 실패를 죽음으로 보면 **네트워크가 한 번 끊긴 밤이 통째로 죽는다.** 그래서 판정 못 한
사유를 `tally` 가 밤마다 남긴다. 분포가 쌓이면 그때 이 가정을 다시 본다 —
...
class Liveness(StrEnum):
    ...
    # 죽음과 갈라 두는 이유는 **대응이 다르기 때문**이다 — 죽음은 그 밤을 막고, 이건 통과시킨다
    UNKNOWN = "unknown"
...
# 한 URL 을 기다리는 시간. 한 밤의 출처가 [실측 2026-09-12] 12건이었고 단계 하나가
# 172~273초 걸리므로, 최악(전부 타임아웃)이어도 밤 전체에서 무시할 수 있는 몫이다
PROBE_TIMEOUT_SECONDS: Final = 10.0

# 자기를 밝히는 UA 를 쓴다. 브라우저를 흉내 내지 않는다 —
# 차단당하면 「판정 못 함」이 되어 게이트가 약해질 뿐, 밤이 죽지는 않는다
...
def shortfall_reason(probed: Mapping[str, Probe]) -> str | None:
    ...
    Returns:
        ...
        「거부됨」만 돌려주면 다음 밤이 무엇을 고쳐야 하는지 모른다
    """
