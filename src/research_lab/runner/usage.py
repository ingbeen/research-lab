...
class Tokens:
    ...
    def all_total(self) -> int:
        ...
        부정했다(모듈 머리의 표). 회차 로그에는 이 합이 아니라 성분 넷과 `tokens_new_total` 이
        실리므로, 가중치가 드러나면 **그 성분에서** 다시 계산한다.
        """
...
class LimitCalibration:
    """사람이 재서 넣은 「한 창의 한도」. 모델을 바꾸면 다시 잰다.

    [중요] **분모는 러너가 읽을 수 없다.** 응답에 한도 필드가 없고, 사용률을 보여 주는
    `/usage` 는 [실측 2026-09-23] **사람의 로그인으로만** 사용률을 내고 회차가 쓰는 장기
    토큰으로는 그 세션의 비용 요약만 낸다. 그래서 사람이 회차 «전»과 «후»의 사용률을 읽어
    그 차이로 한 쌍을 만든다.
...
# 이 계정·이 기계·지금 쓰는 모델의 보정값.
#
# [실측 2026-09-23] 창 경계에 걸리지 않은 회차의 전후 사용률(13% → 40%)과 새 토큰 797,582 로
# 역산했다. 사용률이 정수 % 라 차이가 26 ~ 28%p 사이이고 창은 **2,848,507 ~ 3,067,623**(약 ±4%)
# 이다. 그 범위 안의 아래쪽 값을 반올림해 쓴다 — 아래쪽이라 비율이 조금 크게 나오는 편이지만
# 보증은 아니다. 자릿수를 더 살리지 않는 것은 **정밀해 보이는 숫자를 만들지 않기** 위해서다.
# 같은 날 한 번 더 쟀지만 그 회차는 5시간 창 리셋을 가로질러 참고만 했다.
# 그 전 값(2026-09-15, 1,900,000)은 모델을 바꾸기 전에 잰 것이다 — 두 값의 차이가 모델
# 때문인지 한도 정책 때문인지는 가르지 못했다. **모델을 바꾸면 이 값도 다시 잰다.**
#
# [중요] **오차는 양쪽으로 난다.** 재는 쪽 세션의 요청을 구간 밖으로 뺐으므로, 그 몫이 창을
# «작게» 잡아 비율을 늘 크게(안전하게) 만들던 치우침이 없다.
#
# [중요] `None` 이면 한도 비율은 **「잴 수 없음」**이고 «0 이 아니다». 0 으로 내면
# 「한도를 안 썼다」로 읽히고, 「잴 수 없었다」와 「0이었다」를 구별하지 못하는 계측은
# 계측이 아니라 잡음이다.
#
# 다시 재는 법: 사람의 로그인으로 `claude -p "/usage"` 를 부르면 사용률이 나오고 모델은
# 안 불린다(비용 0). 회차 «직전» 값 → 회차 → «직후» 값을 **스크립트 하나로 묶어** 재는 쪽의
# 요청이 그 사이에 끼지 않게 한다. 그 차이와 그 회차의 `new_total` 로 한 창의 토큰 수를
# 역산해 여기 적고 `measured_on` 을 갱신한다. **5시간 창의 리셋을 가로지른 회차는 쓰지 않는다** —
# 리셋 직전 값을 따로 읽어도 도는 중인 단계의 몫이 어디까지 들어갔는지 알 수 없다
# (절차는 docs/OPERATIONS.md 5.2 절)
#
