...
원장은 **중복 방지의 전부**다. 없으면 같은 후보를 회차마다 다시 판다.
이 저장소가 누적 상태로 «만드는» 것 중 하나인 이유는 하루를 걸러도 그 회차가 없었을 뿐
...
class Status(StrEnum):
    ...
    REJECTED = "rejected"
    # 회차마다 같은 자리에서 막혀 더 해봐야 소용없다고 접은 것.
    #
...
def append(path: Path, claim: str, identifier: str | None = None) -> bool:
    ...
    # 통째로 다시 쓰지 않고 «덧붙인다». 탐색 회차는 한 번에 여러 줄을 담는데,
    # 다시 쓰면 직전 회차가 담은 것과 사람이 적은 메모가 사라지고 **예외도 나지 않는다**
...
def mark_explored(path: Path, claim: str) -> None:
    ...
    [중요] 이것은 **회차의 마지막 단계가 끝난 뒤에** 불려야 한다. 수집이 끝나자마자 표시하면,
...
def mark_blocked(path: Path, claim: str, reason: str) -> None:
    ...
    같은 실행 폴더에서 같은 단계가 회차마다 실패할 때 부른다. **기각과 다르다** —
    잴 수 없다고 판정한 것이 아니라 판정에 닿지도 못한 것이고, 그래서 원인을 고치면
    다시 팔 가치가 있다.

    걷어내지 않으면 다음 회차가 같은 후보를 다시 잡아 **탐색도 수집도 영영 다시 돌지 않고
    회차마다 호출만 한 번씩 태운다.** 나중에 보면 「실패」가 아니라 「아무 일 없음」처럼 보인다.
...
def mark_rejected(path: Path, claim: str, reason: str) -> None:
    ...
    Args:
        ...
        reason: 왜 기각했나. 「거부됨」만 남으면 다음 회차가 같은 시도를 반복한다
...
def status_of(path: Path, claim: str) -> Status | None:
    ...
    이것이 필요한 곳은 회차의 마지막 단계다 — 그 회차의 후보가 **이미 판 것이면** 더 물을
