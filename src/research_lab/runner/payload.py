...
| 자리 | 무슨 일이 나나 |
| --- | --- |
| 후보 목록 | `"없음"` 이 후보 `'없'` 과 `'음'` 둘로 원장에 담긴다. 원장은 append-only 이고 **중복 방지의 전부**라 그 쓰레기가 이후 매일 밤 호출을 한 번씩 잡아먹는다 |
| 검색어 목록 | `"1월 효과"` 가 글자 다섯으로 세어져 **검색어 게이트를 통과한다.** 한 번도 안 갈아 끼운 밤이 「조사한 밤」으로 기록된다 |
