...
[실측 2026-09-12] `--output-format json` 의 응답 모양과 한도 소진 시의 실패 모양은
실측됐다(그 원문은 `tests/test_agent_response_shape.py` 가 고정한다). 그래도 파싱은
**아는 열쇠만 꺼내 보고 없으면 None 으로 두며, 원문은 언제나 통째로 들고 나간다** —
CLI 가 모양을 바꾸는 날 그 원문이 답이 된다.
"""
...
class AgentResult:
    ...
    **그때도 파이프라인은 멈추지 않아야 한다.**
    ...
    #
    # [중요] 이 계층은 **판정하지 않고 옮긴다.** 위 `tokens` 는 캐시에서 읽은 토큰을 뺀
    # 합계이고, 합계만 남기면 가중치가 바뀌는 날 다시 계산할 수 없다. 어느 성분이 한도에서
    # 몇으로 세는지는 이 계층이 알 일이 아니다 — 뜻을 붙이는 것은 러너의 일이다(계층 계약 §2)
...
class StepFailed(RuntimeError):
    ...
    모르면 None 이다 — 지어낸 0 은 「재서 0」으로 읽힌다.
...
def invoke(
    *,
    ...
    try:
        ...
        # 인코딩을 적는 이유는 표준 입력이 로캘을 따르기 때문이다 — 인자일 때는 UTF-8 로 넘어갔다
